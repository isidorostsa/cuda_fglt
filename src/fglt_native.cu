...
// BAD IDEA - this is not a good way to do this
// THE CORRECT WAY IS TO USE A CUSTOM CSR MATRIX STRUCTURE
// THAT HANDLES MEMORY MANAGEMENT SCOPEWISE
cusparseSpMatDescr_t matMul(cusparseHandle_t handle, cusparseSpMatDescr_t A, cusparseSpMatDescr_t B) {
    cusparseSpGEMMDescr_t spgemmDesc;
    CHECK_CUSPARSE(cusparseSpGEMM_createDescr(&spgemmDesc))

    // get A and B sizes
    int64_t A_rows, A_cols, A_nnz;
    CHECK_CUSPARSE(cusparseSpMatGetSize(A, &A_rows, &A_cols, &A_nnz))
    int64_t B_rows, B_cols, B_nnz;
    CHECK_CUSPARSE(cusparseSpMatGetSize(B, &B_rows, &B_cols, &B_nnz))

    int64_t C_rows = A_rows, C_cols = B_cols;
    ...
    int64_t throwaway, C_nnz;
    CHECK_CUSPARSE(cusparseSpMatGetSize(C, &throwaway, &throwaway, &C_nnz));

    std::cout << "C_rows: " << A_rows << std::endl;
    std::cout << "C_cols: " << B_cols << std::endl;
    ...
    CHECK_CUDA(cudaMalloc((void**)&d_C_offs, (C_rows + 1) * sizeof(int)));
    CHECK_CUDA(cudaMalloc((void**)&d_C_cols, C_nnz * sizeof(int)));
    CHECK_CUDA(cudaMalloc((void**)&d_C_vals, C_nnz * sizeof(float)));
...
int main(int argc, char *argv[])
{
    ...
    // HOST DATA
    h_csr h_sA = loadFileToCsr("/work_dir/datasets/test.mtx");
    thrust::host_vector<float> h_sA_vals(h_sA.nnz, 1.0f);

    const int n = h_sA.rows;
